 ...
 * the server — reading the httpOnly cookie, decrypting + JSON-parsing it, and
 * returning the value. On the server you call it exactly the same way.
 *
 * It delegates to the low-level `getCookies` in `cookieOperations.js`
 * (single source of truth for the actual read/decrypt logic).
 *
 * @param {string} name  cookie key — see `@/_config/cookie/configConst`.
 * @returns the decrypted, JSON-parsed value, or `null` if the cookie is absent.
 *
